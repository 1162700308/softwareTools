...
public class SaleOrderServiceImpl implements SaleOrderService {
	...
	public Vector<Vector> selectByCondition(Object[] paraArray) throws Exception {
		Vector<Vector> rows = new Vector<Vector>();//行
		...
		String name = paraArray[0].toString().trim();//参数，trim删除空白字符
		if (!name.isEmpty()) {
		...
		List<Object[]> list = dao.select(sql, 9, null);//sql语句如上，9代表的是列数，也就是几个objec为一行。
		if (!list.isEmpty()) {
